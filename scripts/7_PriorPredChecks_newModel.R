### 7_PriorPredChecks.R
# Ignore data and see what is generated by priors alone
# O.L. Pescott
# Dec. 2019/Jan. 2020
rm(list=ls())
######################################
...
Data <- list(N = N,
             ...
             yearZ = yearZ,
             #y = y,
             y =  rep(NA, length(y1)), # for PPP
             #x2 = x2, # grazing (with NAs)
             x2 = rep(NA, length(x2)), # for PPP
...
#x <- rnorm(n=1000, 0, sd = sqrt(1/0.01)) # tau = 0.1
#s <- rgamma(n=1000, shape = 1/2, rate = 1/2) # k = 1; shape = rate = k/2 = 1/2
#y <- x/sqrt(s)
#hist(abs(y), breaks = 1000, xlim = c(0,10))

# cf. to used in Kruschke (much lower density around 0-50 and much longer RHS tail): pg.496
#x <- abs(rnorm(n=1000, 0, sd = sqrt(1/0.001))) # tau = 0.001
#s <- rgamma(n=1000, shape = 2/2, rate = 2/2) # k = 2; shape = rate = k/2 = 1/2
#y <- x/sqrt(s)
#hist(y, breaks = 10000, xlim = c(0,500))

# Visualise priors for cPos
#pMu <- rbeta(n = 1000, shape1 = 1, shape2 = 1)
#pMu2 <- runif(n = 1000, min = 0, max = 1)
#pX <- abs(rnorm(n=1000, 0, sd = sqrt(1/0.1))) # 
#pS <- rgamma(n=1000, shape = 1/2, rate = 1/2) # k = 1; shape = rate = k/2 = 1/2
#pPhi <- pX/sqrt(pS)
#hist(pPhi)
#pA <- pMu * pPhi
#pB <- (1 - pMu) * pPhi
...
###
par(mfrow=c(3,3))
# half Cauchy, scale = 0.1
sD <- rtt(n = 1000, location = 0, scale = 0.1, df = 1, left = 0, right = Inf) # Irvine etc.
mU <- logit(rbeta(1000,1,1))
hist(rnorm(n = 1000, mean = mU, sd = sD))
hist(log(rnorm(n = 1000, mean = mU, sd = sD)))
hist(ilt(rnorm(n = 1000, mean = mU, sd = sD)))
# half Cauchy, scale = 0.001
sD <- rtt(n = 1000, location = 0, scale = 0.001, df = 1, left = 0, right = Inf) # Irvine etc.
mU <- logit(rbeta(1000,1,1))
hist(rnorm(n = 1000, mean = mU, sd = sD), breaks = 500)
hist(log(rnorm(n = 1000, mean = mU, sd = sD)))
hist(ilt(rnorm(n = 1000, mean = mU, sd = sD)))
# pareto
library(VGAM)
...
hist(log(rnorm(n = 1000, mean = mUp, sd = sDp)))
hist(ilt(rnorm(n = 1000, mean = mUp, sd = sDp)))

##
# half Cauchy, scale = 1, df = 3
# thinner tails, recommended by Aki Vehtari when data are sparse
sD <- rtt(n = 1000, location = 0, scale = 1, df = 3, left = 0, right = Inf) # Irvine etc.
mU <- logit(rbeta(1000,1,1))
hist(rnorm(n = 1000, mean = mU, sd = sD), breaks = 500)
#hist(log(rnorm(n = 1000, mean = mU, sd = sD)))
hist(ilt(rnorm(n = 1000, mean = mU, sd = sD)))
